...
const Contact = ({ enrollCourse }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setFormData({ name: "", email: "", phone: "", course: "", message: "", extraRequirements: "" });
        Swal.fire({
          title: "Message Sent!",
  ...
  // ── Static data ───────────────────────────────────────────────────────────
  const contactInfo = [
    { icon: Phone, title: "Call Us",   detail: "03002207349",          subdetail: "whatsapp available", gradient: "from-blue-600 to-purple-500" },
    { icon: Mail,  title: "Email Us",  detail: "quransabaq@gmail.com", subdetail: "support available",  gradient: "from-blue-600 to-purple-500" },
    { icon: Clock, title: "Available", detail: "24/7 Support",         subdetail: "All Days of Week",   gradient: "from-blue-600 to-purple-500" },
  ];
  ...
  const faqs = [
    {
      ...
      answer: "We offer comprehensive Quran education programs including Quran Reading (Nazra), Quran Memorization (Hifz), Tajweed Mastery, Tafseer & Translation, Islamic Studies, and a specialized Kids Quran Program. Each course is designed to meet the needs of students at every level.",
      gradient: "from-blue-600 to-purple-700",
    },
    {
      icon: Clock3,
      question: "How flexible are the class timings?",
      answer: "We offer highly flexible scheduling with classes available 7 days a week, 24/7. You can choose timings that fit your schedule, and our teachers will accommodate your preferred time slots. We understand that our students are from different time zones worldwide.",
      gradient: "from-blue-600 to-purple-700",
    },
    {
      icon: Gift,
      question: "Do you offer a free trial class?",
      answer: "Absolutely! We offer a complimentary trial class so you can experience our teaching methodology and interact with our teachers before enrolling. No credit card or payment is required for the trial. Simply contact us to schedule your free trial session.",
      gradient: "from-blue-600 to-purple-700",
    },
    {
      icon: Laptop,
      question: "What equipment do I need for online classes?",
      answer: "You'll need a computer, tablet, or smartphone with a stable internet connection, a webcam, and a microphone or headset. We use user-friendly video conferencing platforms that work seamlessly across all devices. Technical support is available if you need assistance with setup.",
      gradient: "from-blue-600 to-purple-700",
    },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Contact Info Cards */}
        <div className={`grid sm:grid-cols-3 gap-8 mb-16 max-w-5xl mx-auto transform transition-all duration-1000 delay-200 ${isVisible ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"}`}>
          {contactInfo.map((info) => (
            <div key={info.title} className="bg-white rounded-2xl p-6 shadow-lg hover:shadow-2xl transform hover:-translate-y-2 transition-all duration-500 border border-gray-100">
              <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${info.gradient} flex items-center justify-center mb-4 transform hover:rotate-12 transition-all duration-300`}>
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          <div className={`transform transition-all duration-1000 delay-400 ${isVisible ? "translate-x-0 opacity-100" : "-translate-x-10 opacity-0"}`}>
            <div id="contact-form" className="bg-gray-50 rounded-3xl p-8 shadow-xl">
              ...
              {enrollCourse && (
                <div className="mb-6 flex items-start gap-3 bg-blue-50 border border-blue-200 rounded-2xl px-4 py-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-blue-700">Course details auto-filled!</p>
                    <p className="text-xs text-blue-500 mt-0.5">
                      Enquiring about: <strong>{enrollCourse.courseName}</strong> — {enrollCourse.schedule}
                    </p>
                  </div>
                </div>
              )}
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="Online Quran Course">Online Quran Course (Pricing Plan)</option>
                  </select>
                </div>

                {/*
                  ─────────────────────────────────────────────────────────────
                  CONDITIONAL FIELDS — this is the key logic:

                  enrollCourse = true  (came from pricing button)
                    → Field 5: Selected Plan Details  (read-only, blue, auto-filled)
                    → Field 6: Additional Requirements (editable, optional)
                    → TOTAL: 6 fields

                  enrollCourse = false/null  (manual form fill)
                    → Field 5: Message  (normal editable textarea)
                    → Field 6: does NOT render at all
                    → TOTAL: 5 fields
                  ─────────────────────────────────────────────────────────────
                */}
          ...
          <div className={`transform transition-all duration-1000 delay-600 ${isVisible ? "translate-x-0 opacity-100" : "translate-x-10 opacity-0"}`}>
            <div className="h-full flex flex-col space-y-6">
              ...
              {/* WhatsApp */}
              <a href="https://wa.me/923002207349" target="_blank" rel="noopener noreferrer" className="block">
                <div className="bg-green-50 border-2 border-green-200 rounded-3xl p-8 hover:border-green-400 transition-all duration-300 cursor-pointer transform hover:scale-105">
                  ...
                  <p className="text-gray-700 mb-4">Get instant answers to your questions on WhatsApp</p>
              ...
              <div className="bg-gray-50 rounded-3xl p-8">
                ...
                <div className="space-y-4">
                  {faqs.map((faq, index) => (
                    <div key={index} className="border border-gray-200 rounded-2xl overflow-hidden bg-white shadow-sm hover:shadow-md transition-all duration-300">
                      <button
                      ...
                      >
                        <div className="flex items-center space-x-4">
                          <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${faq.gradient} flex items-center justify-center flex-shrink-0`}>
                            <faq.icon className="w-5 h-5 text-white" />
                          </div>
                          <span className="font-semibold text-gray-900 text-left">{faq.question}</span>
                        </div>
                        <ChevronDown className={`w-5 h-5 text-gray-500 transition-transform duration-500 flex-shrink-0 ${openFaq === index ? "rotate-180" : ""}`} />
                      </button>
                      <div
